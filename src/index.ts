 ...
 * @function
 */
export let enableMemoization: () => void | Promise<void> = async () => {
	enableMemoization = () => {
		// Convert to a noop so as to not enable memoization multiple times
	};
	const { memoize } = await import('lodash');
	compare = memoize(compare, (a: string, b: string) => `${a}&${b}`);
};
